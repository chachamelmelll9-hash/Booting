...
Scenario: 휴대폰이 없는 부모님과 대면으로 동의한다
  ...
  When 사용자가 "부모님과 함께 이 화면에서 동의" 를 탭하고
       ...
       "동의 완료" 를 탭한다
  Then 화면에서 다음을 확인한다:
    - "실제 성명은 김OO 형태로 가려서 표시됩니다" 문구가 동의 본문에 있었음
    - 프로필 작성 단계로 이동
```
...
#### S5.4: Happy Path — 사진 최대 5장 / 대표 지정

```gherkin
Scenario: 사진은 5장까지, 대표는 1장이다
...
Scenario: 넘긴 프로필은 재추천에서 제외된다
  ...
  [검증: uiautomator dump 에서 해당 마스킹 성명 부재]
```
