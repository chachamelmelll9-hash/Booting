...
#### SAJU.4: 사주가 없는 분은 뒤로 밀릴 뿐 빠지지 않는다

```gherkin
Scenario: 선택 입력이 자격 요건이 되지 않는다
  Given 조건에 맞는 프로필 중 일부는 사주를 등록하지 않았다
  When GET /api/discovery 를 호출한다
  Then 사주가 없는 프로필도 응답에 들어 있다
  And 그 프로필들은 점수가 있는 프로필 **뒤에** 온다
  And 그 카드에는 궁합 배지가 그려지지 않는다
```

#### SAJU.5: 내 부모님 사주가 없으면 최근 활동 순
