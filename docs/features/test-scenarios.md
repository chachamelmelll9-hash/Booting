...
| 16 | 매칭 목록 (상태 추적) | 3 | S16.1 ~ S16.3 | Mobile, Server |
| 17 | 부모님 의사 확인 → **매칭** | 3 | S17.1 ~ S17.3 | Mobile, Server, DB |
| 18 | 만남 일정 (+ 미동행) — API 전용 | 5 | S18.1 ~ S18.5 | Server, DB |
| 19 | 만남 확인 (양측) — API 전용 | 4 | S19.1 ~ S19.4 | Server, DB |
...
- [ ] **Step 12: 신고 / 차단**
  - [ ] S12.1: 신고 접수 → 목록에서 제거
  - [ ] S12.2: 차단 → 추천 제외
  - [ ] S12.3: 대화방 ⋯ 신고 → 신고 내역에 사유 표시
  - [ ] S12.4: 새 대화방 → 매칭 탭 민트 배지 + 카드 민트 테두리, 열면 둘 다 꺼짐
  - [ ] S12.5: 매칭 카드 `부모님께 공유` → **카카오톡만** 열림(OS 시트 없음) → 전송하면 콜백으로 카드 회색 + `부모님께 공유 완료`, 대화방에 `{내 부모님 별명} 님의 자녀가 프로필을 공유했습니다.` 한 줄
  - [ ] S12.5b: 카카오톡을 열었다가 **보내지 않고** 나오면 `공유 완료`가 되지 않는다 (기다림 문구만 뜨고 버튼으로 돌아온다)
  - [ ] S12.6: 받은 관심 `찜해놓기` → 보관함에 쌓임, 받은 관심에서 빠짐, `찜 풀기`로 돌아옴
  - [ ] S12.7: 카카오 계정 연결 — 이메일로 로그인 → `내 정보 > 계정 > 카카오 계정 연결` → 로그아웃 → 카카오 로그인 → **같은 계정**으로 진입 (새 계정이 생기지 않는다)
  - [ ] S12.8: 연결 해제 후 카카오 로그인 → 별도 계정으로 진입한다 (연결이 실제로 끊겼다)
  - [ ] S12.9: 이미 다른 계정에 연결된 카카오를 붙이려 하면 거절된다 (`kakao_already_linked`)
- [ ] **Step 13: 받은 하트**
  - [ ] S13.1: 미읽음/읽음 구성 차이
...
- [ ] **Step 17: 부모님 의사 확인**
  - [ ] S17.1: 동의 선택 → 대기 상태 ('매칭 성공' 문구 부재 검증 포함)
  - [ ] S17.2: 거절 선택 → 확인 후 인연 종료
  - [ ] S17.3: 양측 동의 → **매칭 성공** (`connections.status='matched'`)
- [ ] **Step 18: 만남 일정**
...
- [ ] 매칭 탭 진입 → 목록 또는 빈 상태 렌더 (헤더가 '매칭')
- [ ] 대화방에서 뒤로가기(헤더 화살표·하드웨어 모두) → 매칭 목록 복귀 (홈 탭으로 튀지 않음)
...
#### S12.3: Happy Path — 대화방에서 신고하고 내역에서 확인

```gherkin
Scenario: 대화 상대를 신고하면 고른 사유가 신고 내역에 남는다
  Given 사용자가 대화 중인 대화방에 들어와 있다
  When 사용자가 우상단 ⋯ → "대화 상대 신고하기" → "사기 의심" 선택 후 "신고하기" 를 탭한다
  Then "신고가 접수되었습니다" 토스트가 표시되고 시트가 닫힌다
  And 매칭 목록으로 돌아가고 그 대화가 목록에서 사라진다 (ended 카드로도 남지 않는다)
  And 내 정보 → "신고 내역" 에 상대 별명 · "사기 의심" · "접수됨" 이 한 줄로 뜬다
  And 홈 추천에도 그 프로필이 다시 나오지 않는다
```
...
#### S17.3: Happy Path — 양측 동의 = 매칭 성공

```gherkin
Scenario: 양쪽 모두 동의하면 그 자리에서 매칭된다
  Given 상대측이 이미 "만나보고 싶다" 로 응답했다
  When 사용자가 "만나보고 싶다고 하세요" 를 선택하고 제출한다
  Then 채팅방 상태 배지가 "매칭 성공" 으로 바뀐다
  And 매칭 탭 '매칭' 필터에 이 인연이 나타난다
  And DB `connections.status = 'matched'`, `matched_at` 이 채워진다
```

**한쪽만 답한 시점의 부재 검증** — S14.2 와 같은 방식이다:

```gherkin
Scenario: 한쪽 답변만으로는 매칭이 아니다
  Given 상대측이 아직 응답하지 않았다
  When 사용자가 "만나보고 싶다고 하세요" 를 선택하고 제출한다
  Then 상태가 "부모님 의사 확인" 이다
  And 화면 어디에도 "매칭 성공" 문구가 없다
  [검증: uiautomator dump 전체 grep 으로 부재 확인]
```

---

### Step 18: 만남 일정 (+ 미동행) — **API 전용**

> Depends: Step 17
> **앱 동선에서 뺐다.** 약속 시간·장소는 채팅으로 정하고, 매칭은 Step 17 에서 끝난다
> (`first-meeting-match.md` 의 '앱 동선에서 뺀 것' 참고).
> 아래 시나리오는 **서버 계약 검증용**으로만 유지한다 — 대응 화면이 없으므로 ADB 로는
> 돌리지 않고 `scripts/smoke-api.mjs` [11] 절이 같은 내용을 검증한다.
> 관련 화면(라우트만 존재): `app/(tabs)/connections/[id]/meeting.tsx`, `meeting-solo.tsx`
...
### Step 19: 만남 확인 (양측) — **API 전용**

> Depends: Step 18
> **앱 동선에서 뺐다** (Step 18 참고). 매칭 판정은 Step 17 로 옮겼고, 여기서는
> 만남 기록(`meetings.status`)만 진행한다. `matched` 는 종착점이라 이 API 를
> 어떻게 부르더라도 인연 상태가 되돌아가지 않는다 — `smoke-api.mjs` [11] 이 검증한다.
> 관련 화면(라우트만 존재): `app/(tabs)/connections/[id]/meeting-confirm.tsx`
...
#### SAJU.1: 모든 공개 프로필에 사주가 있다

```gherkin
Scenario: 사주 보정값을 적지 않아도 궁합이 나온다
  Given 상대 프로필에 saju_infos 행이 없다 (생년월일만 있다)
  When GET /api/discovery 를 호출한다
  Then 그 상대의 dayPillar 가 채워져 있다
  And compatibility 가 채워져 있다
  # 생년월일은 필수라 양력·시각 모름으로 세 기둥을 세울 수 있다.
  # saju_infos 는 보정값이지 사주의 유무를 가르는 스위치가 아니다

Scenario: 팔자를 못 세우면 그때만 null 이다
  Given 상대의 생년월일이 음력 변환표(1391~2050) 밖이다
  When GET /api/discovery 를 호출한다
  Then 그 상대의 compatibility 가 null 이다
  And 카드에 궁합이 그려지지 않는다   # 0점도, '측정 불가'도 아니다
```
...
#### SAJU.3: 조건에 맞으면 언제나 궁합 높은 순

```gherkin
Scenario: 사용자가 고르지 않아도 궁합 순으로 온다
  Given 내 부모님 사주가 등록돼 있다
  When GET /api/discovery 를 호출한다
  Then items 가 compatibility.score 내림차순이다
  And 추천 조건 화면 어디에도 정렬 선택지나 최소 궁합 조건이 없다
  # 궁합은 조건이 아니라 순서다 — "궁합 낮은 순" 은 뜻이 없는 선택지다
```

#### SAJU.4: 점수가 없어도 추천에서 빠지지 않는다

```gherkin
Scenario: 궁합을 못 내는 것이 제외 사유가 되지 않는다
  Given 조건에 맞는 프로필 중 하나는 팔자를 못 세운다 (날짜 손상·변환표 밖)
  When GET /api/discovery 를 호출한다
  Then 그 프로필도 응답에 들어 있다
  And 점수가 있는 프로필 **뒤에** 온다
  And 그 카드에는 일주와 궁합이 그려지지 않는다
```

#### SAJU.5: 내 팔자를 못 세우면 최근 활동 순

```gherkin
Scenario: 예외 상황에서도 홈이 비지 않는다
  Given 내 부모님 생년월일로 팔자를 세울 수 없다
  When GET /api/discovery 를 호출한다
  Then 최근 활동 순 추천이 평소와 같은 수로 내려온다
  And nextCursor 가 위치(o:20)가 아니라 타임스탬프다
  And 어느 카드에도 궁합이 없다 (일주는 상대 것이라 그대로 나온다)
```

#### SAJU.6: 사주 입력 화면에 '입력 안 함'이 없다

```gherkin
Scenario: 사주를 끄는 선택지를 두지 않는다
  When 부모님 프로필 작성의 '사주 정보' 섹션을 연다
  Then 생년월일 기준 선택지가 '양력'과 '음력' 둘뿐이다
  And 기본값이 '양력'이다
  # 끄는 선택지를 두면 "사주를 안 쓴 사람" 상태가 생기고 카드가 두 종류로 갈린다
```
...
Scenario: 자녀가 부모님을 등록해 최종 매칭까지 도달한다
  ...
  Then 양측 화면에 "매칭 성공" 이 표시된다
  [스크린샷: E2E-09]

  # 최종 확인
  Then 사용자의 목표가 달성되었다:
    - 양측 인연 목록에 "매칭 성공" 상태 표시
    - connections.status = 'matched'
