...
| SajuInfo | 사주 **보정값** (양·음력, 태어난 시간). 행이 없어도 `parent_profiles.birth_date` 로 팔자를 세우므로 사주의 유무를 가르지 않는다. `is_public` 은 **항상 false** 로 쓴다 — 사용자 선택이 아니라 RLS 정책 `saju_public_read` 를 닫아 두기 위한 값이다. 사주팔자는 저장하지 않고 매번 계산한다 | id, parent_profile_id, birth_date, calendar_type(solar/lunar), birth_time, birth_time_unknown, is_public(always false), created_at | parent-profile-consent, profile-discovery, saju-compatibility |
