...
변경 사항:
- 카카오 **로그인** 버튼은 `EXPO_PUBLIC_KAKAO_LOGIN=true` 일 때만 뜬다. 네이티브 키로 판단하지 않는다 — 그 키는 '부모님께 공유'에도 쓰여서, 공유만 하려고 키를 넣으면 로그인 버튼이 딸려 나온다. 콘솔에서 카카오 로그인을 안 켠 상태면 `KOE004`(서비스 설정 오류)로 끝난다 (실측)
- 콘솔 선행 조건 두 가지: **카카오 로그인 활성화**(없으면 KOE004), **OpenID Connect 활성화**(없으면 `login()` 이 `idToken` 을 주지 않아 Supabase 가 세션을 못 만든다). 둘은 별도 스위치다
- **계정 연결**: 카카오는 이메일을 주지 않아(동의항목이 비즈니스 앱 전용) 이메일로 가입한 사람이 카카오로 들어오면 계정이 하나 더 생긴다. 그래서 `내 정보 > 계정 > 카카오 계정 연결` 에서 로그인한 상태로 한 번 붙여 둔다 — 지금 로그인해 있다는 것 자체가 이 계정의 주인이라는 증거다. 그다음부터 카카오 로그인은 Supabase 보다 **먼저** `POST /auth/kakao/resolve` 로 물어 연결된 계정의 세션을 받는다
