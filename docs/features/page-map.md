...
| 3 | 인연 | 두 사람 연결 아이콘 (배지 표시) | 상태별 인연 목록 | 하트 & 대화 연결 + 첫 만남 & 최종 매칭 |
...
**알림은 탭을 차지하지 않는다.** 보일러플레이트의 `(tabs)/notifications` 라우트는 유지하되
탭바에서 숨기고(`href: null`) 홈 헤더의 종 아이콘으로 진입한다. 탭 5개는 이 앱의 주 동선
(추천 → 관심 → 인연)을 흐리게 만든다.
...
- **Navigation**:
  ...
  - Back: N/A (tab root)
- **Reuse**: New
- **Note**: 상태 필터 칩 = PRD 10.3의 상태 문구를 그대로 쓴다 (서로 관심이 있어요 / 대화 중이에요 / 부모님 의사 확인 중이에요 / 만남을 준비하고 있어요 / 상대방의 만남 확인을 기다리고 있어요 / 최종 매칭 성공)
...
- **Reuse**: New
- **Note**: 최종 매칭 전이 여부는 서버 응답으로만 판단한다. 클라이언트가 '최종 매칭 성공'을 선언하지 않는다
...
- **Source Feature**: 하트 & 대화 연결 / 첫 만남 & 최종 매칭 확정
- **Data Source**: Server API
- **Navigation**: From 홈 헤더 종 아이콘 / To 알림 종류별 대상 화면 / Back 홈
...
[Tab 관심: 받은 하트] ---+                                                      |
                                                                               v
[Tab 인연: 상태별 목록] <--상태 갱신-- [parent-intent] --양측 동의--> [meeting]
        |                                                                |
        +--만남 예정--> [meeting] --동행 안 함--> [meeting-solo]           |
                            |                                            |
                            +--만남 시각 이후--> [meeting-confirm] --양측 확인--> [최종 매칭 성공]
                                                       |
                                                       v
                                                  [feedback (비공개)]
...
|----------|--------|--------|
| 탭 개수·구성 | 4개 (홈·관심·인연·내 정보) | 메시지를 인연에 합쳐 "지금 어느 단계인가" 판단을 한 화면에 모았다 |
| 메시지 전용 탭 | 두지 않음 | PRD 13.5·13.6의 대상이 동일 — 목록 행이 상태 배지 + 마지막 메시지를 함께 보여준다 |
| 알림 위치 | 탭이 아니라 홈 헤더 종 아이콘 | 탭 5개는 주 동선(추천→관심→인연)을 흐린다. 보일러플레이트 라우트는 `href: null`로 유지 |
