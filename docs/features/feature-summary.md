...
5. **사주 궁합** — 1의 SajuInfo 를 소비해 2·3의 카드와 상세에 얹는다 (독립 화면 없음)
...
- **Key Decisions**: 부모님 1명(TODO-07) / 만 50세 이상(TODO-02) / 휴대폰 없으면 자녀 단말 대면 동의(TODO-04) / 가족관계 심사는 MVP 자동 승인이되 상태 기계는 실심사 교체 가능하게 유지(TODO-05) / 사주는 선택 입력 + 공개 여부 선택, **궁합은 무료로 제공**(TODO-13, 2026-09-09 P2→MVP) — 사주 생년월일은 기본정보의 날짜 하나만 쓰고 양·음력만 고른다 / 사진 **최소 3장**·최대 5장·대표 1장 필수 / 가족·생활 정보 7항목(자녀 수·동거 가족·종교·직업·음주·흡연·취미) 모두 필수 / 동거 가족은 4개 보기 복수 선택, 흡연은 비흡연/흡연 단일 선택 / 부모님 최종 승인 없이는 어떤 경로로도 공개되지 않는다
...
- **Key Decisions**: 실시간 GPS 대신 설정된 생활권 사용 / 하트는 확인 다이얼로그 없이 즉시 전송(중복만 차단) / 넘기기 되돌리기 미제공 / 스와이프 전용 조작 금지 — 하트·넘기기는 버튼으로도 제공 / 상세에서 가장 먼저 읽혀야 할 것은 자녀가 쓴 소개글 / **'동성 친구' 선택 시 같은 성별이면서 동성 친구를 찾는 프로필만 추천** (성별 필터보다 우선), 반대로 목적이 '동성 친구' 하나뿐인 프로필은 이성에게 추천되지 않는다 / 정렬은 최근 활동 순이 기본이고 **궁합 좋은 순**을 고를 수 있다 — 우리 부모님 사주가 없으면 궁합 조건은 무시하고 최근 활동 순으로 돌려준다

### 사주 궁합

- **Source Spec**: `docs/features/saju-compatibility.md`
- **User Goal**: 상대 부모님이 우리 부모님과 사주로 어떻게 맞는지 함께 보고 관심을 보낼지 정한다
- **Summary**: 양쪽 사주로 사주팔자 네 기둥을 세워 궁합 점수(30~99)·등급·근거를 낸다. **전면 무료**이고 외부 유료 API 없이 서버가 직접 계산한다. 독립 화면을 두지 않고 추천 카드·받은 관심 카드·상세 프로필·추천 조건에 얹는다. 양쪽 다 사주를 입력했을 때만 나오며, 없으면 배지 자체를 그리지 않는다.
- **Journey Steps**: 입력(프로필 작성의 선택 항목) → 발견(카드 배지) → 결과(상세의 점수·근거·네 기둥) → 후속(궁합 좋은 순 정렬·최소 궁합)
- **Key Screens**: `/(parent-setup)/profile-edit`, `/(tabs)/home`, `/(tabs)/hearts`, `/profile/[id]`, `/(tabs)/home/filters`
- **Core Data**: SajuInfo, DiscoveryFilter(`sort`, `min_compatibility`)
- **Key Decisions**: 무료 고정(TODO-10) / 연주는 입춘·월주는 12절 기준 / 1954~1961 표준시(UTC+8:30) 30분 보정 / 출생시각을 모르면 시주를 빼고 세 기둥으로 보되 **점수를 깎지 않는다**(정규화) / `is_public` 은 생년월일 노출만 가르고 궁합 계산은 그와 무관 / **상대의 네 기둥은 상대가 공개했을 때만** 보낸다 / 서버는 코드만 내리고 한글은 모바일 `shared/config/saju.ts` 한 곳 / 인연 관리 탭에는 넣지 않는다 / 낮은 점수에 '나쁜 궁합'이라 쓰지 않는다
...
3. **민감정보 비공개**: 가족관계증명서 원문, 주민등록번호, 정확한 주소·실시간 위치, 연락처. 실명은 공개하지 않고 사용자가 정한 **별명**으로 표시한다(별명을 실명과 같게 두는 건 본인 선택으로 허용하되, 등록 시 한 번 확인받는다).
4. **자녀 수·동거 가족은 검색 필터로 쓰지 않는다.** 상세 프로필에서만 노출.
5. **전면 무료** (TODO-10). 인앱결제·AdMob 관련 UI를 만들지 않는다. 다만 안내 문구는 `전면 무료입니다.`까지만 쓰고 **'광고 없음'은 문구로 노출하지 않는다**.
