...
- **Key Decisions**: 무료 고정(TODO-10) / **등급 문구 없음** — 숫자는 참고로 읽히지만 문구는 결론으로 읽힌다 / 카드 표기는 `별명·나이` → `을사일주`(본인 것) → `궁합 82점` 순, 일주는 60일 주기라 공개 여부와 무관하게 나간다 / **궁합은 조건이 아니라 순서** — 정렬 선택지도 최소 점수도 두지 않는다 (점수로 거르면 선택 입력인 사주가 자격 요건이 된다) / 사주 없는 프로필은 뒤로 밀릴 뿐 빠지지 않는다 / 연주는 입춘·월주는 12절 기준 / 1954~1961 표준시(UTC+8:30) 30분 보정 / 출생시각을 모르면 시주를 빼고 세 기둥으로 보되 **점수를 깎지 않는다**(정규화) / `is_public` 은 생년월일 노출만 가르고 궁합 계산은 그와 무관 / **상대의 네 기둥은 상대가 공개했을 때만** 보낸다 / 서버는 코드만 내리고 한글은 모바일 `shared/config/saju.ts` 한 곳 / 인연 관리 탭에는 넣지 않는다 / 낮은 점수에 '나쁜 궁합'이라 쓰지 않는다
