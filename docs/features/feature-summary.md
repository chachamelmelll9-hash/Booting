...
- **Summary**: 양쪽 사주로 사주팔자 네 기둥을 세워 궁합 점수(30~99)·등급·근거를 낸다. **전면 무료**이고 외부 유료 API 없이 서버가 직접 계산한다. 독립 화면도 조작 장치도 두지 않는다 — 추천 카드·받은 관심 카드·상세 프로필에 얹고, 조건에 맞는 분들을 궁합 높은 순으로 정렬한다. 양쪽 다 사주를 입력했을 때만 점수가 나오며, 없으면 배지 자체를 그리지 않는다.
