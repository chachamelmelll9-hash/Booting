...
- 13:4x "웰컴 애니메이션 물병 말고 하트병" → SVG 하트 경로 클리핑으로 차오르게. "병 입구 밤티야 ㅋㅋ" → 유리 목 → "목 없애줘" → "뚜껑 날라가는 것도" → "끝나고 남는 작은 하트 없애", "더 많이 쏟아지게"(18→34) — `00e8f35`
- 14:0x "사별/이혼 상태 탭 없애자는 의견" → "민감정보라 오픈하고 싶지 않은 경우는?" → **어느 쪽인지 묻지 않는다.** 1단계에 자격 안내 + "네, 해당됩니다". 옛 버전은 브랜치 `이혼사별여부있음` — `264bbae`, [결정](decisions/2026-09-18-no-marital-status-input.md). 마이그레이션 `20260918140000` 적용 대기
- 14:3x "가족관계증명서 확인 뺐었는데 다시… 남이 올리면 어떡해" → 동의 폰 묶기 → "부모님인지 확인이 안 되잖아" → 수동 심사 → "아무한테나 카톡 보내면?" → "AI 자동 심사로" → "API 키까진 안 쓰고 싶고, 올리면 패스, 적발되면 제한". 3단계에 증명서 업로드(비공개 버킷 보관), 접수 즉시 승인, Claude 대조는 키 있을 때만 — `52988e2`, [결정](decisions/2026-09-18-family-certificate-upload-pass.md)
- 16:0x "사진 등록 아직도 에러" → 옛 서버가 새 앱의 요청(혼인 상태 없음)을 거부한 것. 소유자가 `20260918140000` SQL 을 대시보드에서 적용 → 서버 재기동. 5556 개발 계정으로 **사진 3장 한 번에 선택 → 3/5장, 증명서 1장 → "접수되었습니다"** 까지 확인 (서버 로그 `family doc approved … (auto)`)
