...
| 가족관계증명서 | 비공개 버킷 `family-docs` 에 보관 (09-18 재도입) | 어디에도 노출 안 함. 신고 시 운영자만 확인 | 주민번호 포함 — 스토어 데이터 안전 선언에 반영 필요 |
| 혼인 상태(사별/이혼) | **입력받지 않음** (09-18) | 카드·상세·부모님 화면·필터 전부에서 제거 | 옛 데이터의 `marital_status` 값은 남아 있으나 읽지 않는다 |
