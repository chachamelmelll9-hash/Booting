...
3. 자녀 본인인증(+가족관계 인증)을 완료해야 이용할 수 있다. — 가족관계 인증은 09-04 폐지 → 09-18 재도입(올리면 통과, 적발되면 제한), [결정](../decisions/2026-09-18-family-certificate-upload-pass.md)
4. 등록할 수 있는 부모님의 혼인 상태는 **사별 또는 이혼**으로 제한한다. — 09-18 부터 어느 쪽인지는 묻지 않고 자격 확인만 받는다, [결정](../decisions/2026-09-18-no-marital-status-input.md)
