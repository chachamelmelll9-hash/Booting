...
| `scripts/ensure-emulator.sh` (`.ps1`) | 에뮬레이터 준비의 **유일한 경로**: 소프트웨어 렌더링, 화면 깨우기·**안 잠들게**(배터리 AC 고정, `4ec206e`), AVD 락, 프로세스 분리, 포트 리버스 8081/3000/4200/54321. `ANDROID_SERIAL` 로 대상 지정, `--restart`. **`.ps1` 은 `-Port 5556` 으로 두 번째 에뮬레이터**(09-15: 발표용 가입 화면 `booting_signup`) — 그 시리얼만 다루고 떠 있는 첫 대는 안 건드린다 | 여러 대일 때 시리얼·`--restart` 버그 수정 `1c56708`. 직접 `emulator` 를 띄우지 않는다. 두 대는 발표 때만, 끝나면 바로 내린다(메모리) |
| `scripts/seed-demo.mjs` | 시드 프로필 23+ 생성/갱신(공개된 프로필도 PATCH), `--clean`, `--reset-feed`(demo 의 하트·넘김·인연·상대 하트 삭제), `--heart-me [email|latest]`(이성 시드 전원이 하트 → 받은 관심이 찬다. 기본 demo, 이메일 지정 가능, **`latest` 는 가장 최근 개발용 새 계정** — 발표 시연이 매번 새 계정 가입부터 밟기 때문, 09-14), `--scenarios`(대화 연결/대화 중/매칭 인연 실제 API 로), `--partner-intent`, `--heart <email>`, **`--diagnose`**(dev-login 으로 붙어 프로필/필터/추천/하트를 서버에서 재현 — "앱 문제 vs 서버 문제" 를 한 번에 가른다) | 계정은 service-role admin API 로 만든다(공개 가입은 rate limit). 도메인 `@seed.booting.app`. 데모 비밀번호는 `.launch-demo-account`(gitignore) |
| `scripts/open-parent-web.mjs [connectionId] [--print]` | **부모님 화면**(`/p/:token`)을 PC 브라우저에 띄운다 — 가장 최근에 부모님께 공유된 인연을 골라 서버와 같은 HMAC 토큰을 만든다. 개발 기기 카톡으로는 카드를 못 받으니 시연 때 옆 창으로 (09-14) | `apps/server/.env.development` 의 `PUBLIC_BASE_URL`(터널)로 연다 |
