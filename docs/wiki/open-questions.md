...
- **이 PC 의 `SUPABASE_ACCESS_TOKEN` 이 죽어 있다** (09-18) — 관리 API·CLI(`link`, `migration list`) 모두 401. 마이그레이션은 소유자가 대시보드 SQL 편집기에서 직접 넣어야 한다 (09-18 `20260918100000` 도 그렇게 적용). 새 토큰을 `setx SUPABASE_ACCESS_TOKEN` 으로 등록하면 MCP/CLI 경로가 다시 산다
